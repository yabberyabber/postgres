...
/*
 * sk_bt_restore_page -- re-enter all the index tuples on a page
 *
 ...
 */
static void
sk_bt_restore_page(Page page, char *from, int len)
{
	...
	for (i = nitems - 1; i >= 0; i--)
	{
		if (PageAddItem(page, items[i], itemsizes[i], nitems - i,
			...
			elog(PANIC, "sk_bt_restore_page: cannot add item to page");
		from += itemsz;
	}
}

static void
sk_bt_restore_meta(XLogReaderState *record, uint8 block_id)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	Buffer		metabuf;
	Page		metapg;
	SK_BTMetaPageData *md;
	SK_BTPageOpaque pageop;
	...
	sk_bt_pageinit(metapg, BufferGetPageSize(metabuf));
	...
	pageop = (SK_BTPageOpaque) PageGetSpecialPointer(metapg);
	...
	((PageHeader) metapg)->pd_lower =
		((char *) md + sizeof(SK_BTMetaPageData)) - (char *) metapg;
...
/*
 * sk_bt_clear_incomplete_split -- clear INCOMPLETE_SPLIT flag on a page
 *
 * This is a common subroutine of the redo functions of all the WAL record
 * types that can insert a downlink: insert, split, and newroot.
 */
static void
sk_bt_clear_incomplete_split(XLogReaderState *record, uint8 block_id)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	Buffer		buf;
	...
	if (XLogReadBufferForRedo(record, block_id, &buf) == BLK_NEEDS_REDO)
	{
		...
		SK_BTPageOpaque pageop = (SK_BTPageOpaque) PageGetSpecialPointer(page);
...
btree_xlog_insert(bool isleaf, bool ismeta, XLogReaderState *record)
{
	 ...
	 */
	if (!isleaf)
		sk_bt_clear_incomplete_split(record, 1);
...
btree_xlog_split(bool onleft, XLogReaderState *record)
{
	 ...
	 */
	if (!isleaf)
		sk_bt_clear_incomplete_split(record, 3);
	...
	sk_bt_pageinit(rpage, BufferGetPageSize(rbuf));
	ropaque = (SK_BTPageOpaque) PageGetSpecialPointer(rpage);
	...
	sk_bt_restore_page(rpage, datapos, datalen);
	...
	if (XLogReadBufferForRedo(record, 0, &lbuf) == BLK_NEEDS_REDO)
	{
		 ...
		 * items to that in item number order.  This mirrors how sk_bt_split()
		 ...
		 * but it helps debugging.  See also sk_bt_restore_page(), which does
		 * the same for the right page.
		 */
		Page		lpage = (Page) BufferGetPage(lbuf);
		SK_BTPageOpaque lopaque = (SK_BTPageOpaque) PageGetSpecialPointer(lpage);
		OffsetNumber off;
		Item		newitem = NULL;
		Size		newitemsz = 0;
		Page		newlpage;
...
btree_xlog_mark_page_halfdead(uint8 info, XLogReaderState *record)
{
	...
	if (XLogReadBufferForRedo(record, 1, &buffer) == BLK_NEEDS_REDO)
	{
		...
		page = (Page) BufferGetPage(buffer);
		pageop = (SK_BTPageOpaque) PageGetSpecialPointer(page);
	...
	sk_bt_pageinit(page, BufferGetPageSize(buffer));
	pageop = (SK_BTPageOpaque) PageGetSpecialPointer(page);
...
btree_xlog_newroot(XLogReaderState *record)
{
	...
	{
		ptr = XLogRecGetBlockData(record, 0, &len);
		sk_bt_restore_page(page, ptr, len);
...
btree_xlog_reuse_page(XLogReaderState *record)
{
	 ...
	 * RecentGlobalXmin test in sk_bt_page_recyclable() conceptually mirrors the
...
btree_mask(char *pagedata, BlockNumber blkno)
{
	...
	maskopaq = (SK_BTPageOpaque) PageGetSpecialPointer(page);
	...
	else if (P_ISLEAF(maskopaq))
	{
		 ...
		 * sk_bt_killitems(), sk_bt_check_unique() for details.
		 */
		mask_lp_flags(page);
	}

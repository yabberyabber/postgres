...
static bool sk_bt_mark_page_halfdead(Relation rel, Buffer buf, BTStack stack);
static bool sk_bt_unlink_halfdead_page(Relation rel, Buffer leafbuf,
						 bool *rightsib_empty);
static bool sk_bt_lock_branch_parent(Relation rel, BlockNumber child,
					   BTStack stack, Buffer *topparent, OffsetNumber *topoff,
					   BlockNumber *target, BlockNumber *rightsib);
static void sk_bt_log_reuse_page(Relation rel, BlockNumber blkno,
...
/*
 *	sk_bt_initmetapage() -- Fill a page buffer with a correct metapage image
 */
void
sk_bt_initmetapage(Page page, BlockNumber rootbknum, uint32 level)
{
	SK_BTMetaPageData *metad;
	SK_BTPageOpaque metaopaque;

	sk_bt_pageinit(page, BLCKSZ);
	...
	metaopaque = (SK_BTPageOpaque) PageGetSpecialPointer(page);
	...
	((PageHeader) page)->pd_lower =
		((char *) metad + sizeof(SK_BTMetaPageData)) - (char *) page;
}

/*
 *	sk_bt_getroot() -- Get the root page of the btree.
 *
 ...
 */
Buffer
sk_bt_getroot(Relation rel, int access)
{
	/*Buffer		metabuf;
	Page		metapg;
	SK_BTPageOpaque metaopaque;
	...
	return sk_bt_getbuf(rel, BTREE_METAPAGE, BT_READ);
	#if 0
	/*
	...
	{
		metad = (SK_BTMetaPageData *) rel->rd_amcache;
		/* We shouldn't have cached it if any of these fail */
		Assert(metad->btm_magic == BTREE_MAGIC);
		Assert(metad->btm_version == BTREE_VERSION);
		Assert(metad->btm_root != P_NONE);

		rootbuf = sk_bt_getbuf(rel, rootblkno, BT_READ);
	...
	metabuf = sk_bt_getbuf(rel, BTREE_METAPAGE, BT_READ);
	metapg = BufferGetPage(metabuf);
	metaopaque = (SK_BTPageOpaque) PageGetSpecialPointer(metapg);
	...
	if (metad->btm_root == P_NONE)
	{
		...
		if (metad->btm_root != P_NONE)
		{
			 ...
			 */
			sk_bt_relbuf(rel, metabuf);
			return sk_bt_getroot(rel, access);
		}
	...
	else
	{
		...
		rel->rd_amcache = MemoryContextAlloc(rel->rd_indexcxt,
											 sizeof(SK_BTMetaPageData));
		memcpy(rel->rd_amcache, metad, sizeof(SK_BTMetaPageData));

		/*
		 * We are done with the metapage; arrange to release it via first
		 * sk_bt_relandgetbuf call
		 */
		rootbuf = metabuf;

		for (;;)
		{
			rootbuf = sk_bt_relandgetbuf(rel, rootbuf, rootblkno, BT_READ);
			rootpage = BufferGetPage(rootbuf);
			rootopaque = (SK_BTPageOpaque) PageGetSpecialPointer(rootpage);
...
/*
 *	sk_bt_gettrueroot() -- Get the true root page of the btree.
 *
 *		This is the same as the BT_READ case of sk_bt_getroot(), except
 ...
 */
Buffer
sk_bt_gettrueroot(Relation rel)
{
	Buffer		rootbuf = 0;
	#if 0
	Page		metapg;
...
/*
 *	sk_bt_getrootheight() -- Get the height of the btree search tree.
 *
 ...
 */
int
sk_bt_getrootheight(Relation rel)
{	
	#if 0
	SK_BTMetaPageData *metad;
	 ...
	 * cached yet, load it.  Sanity checks here must match sk_bt_getroot().
	 */
	if (rel->rd_amcache == NULL)
	{
		Buffer		metabuf;
		...
		/*
		 * If there's no root page yet, sk_bt_getroot() doesn't expect a cache
		 * to be made, so just stop here and report the index height is zero.
		 * (XXX perhaps sk_bt_getroot() should be changed to allow this case.)
		 */
		if (metad->btm_root == P_NONE)
		{
			sk_bt_relbuf(rel, metabuf);
...
/*
 *	sk_bt_checkpage() -- Verify that a freshly-read page looks sane.
 */
void
sk_bt_checkpage(Relation rel, Buffer buf)
{
	 ...
	 */
	if (PageGetSpecialSize(page) != MAXALIGN(sizeof(SK_BTPageOpaqueData)))
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
 ...
 */
static void
sk_bt_log_reuse_page(Relation rel, BlockNumber blkno, TransactionId latestRemovedXid)
{
...
/*
 *	sk_bt_getbuf() -- Get a buffer by block number for read or write.
 *
 ...
 *		sk_bt_checkpage to sanity-check the page (except in P_NEW case).
 */
Buffer
sk_bt_getbuf(Relation rel, BlockNumber blkno, int access)
{
	Buffer		buf;
	...
	else
	{
		 ...
		 * rightmost page, so it's entirely likely that sk_bt_split will call us
			...
			if (ConditionalLockBuffer(buf))
			{
				...
				if (sk_bt_page_recyclable(page))
				{
					...
					if (XLogStandbyInfoActive() && RelationNeedsWAL(rel))
					{
						...
						sk_bt_log_reuse_page(rel, blkno, opaque->btpo.xact);
					}

					/* Okay to use page.  Re-initialize and return it */
					sk_bt_pageinit(page, BufferGetPageSize(buf));
					return buf;
				}
...
/*
 *	sk_bt_relandgetbuf() -- release a locked buffer and get another one.
 *
 * This is equivalent to sk_bt_relbuf followed by sk_bt_getbuf, with the
 * exception that blkno may not be P_NEW.  Also, if obuf is InvalidBuffer
 * then it reduces to just sk_bt_getbuf; allowing this case simplifies some
 * callers.
 *
 * The original motivation for using this was to avoid two entries to the
 * bufmgr when one would do.  However, now it's mainly just a notational
 * convenience.  The only case where it saves work over sk_bt_relbuf/sk_bt_getbuf
 * is when the target page is the same one already in the buffer.
 */
Buffer
sk_bt_relandgetbuf(Relation rel, Buffer obuf, BlockNumber blkno, int access)
{
	Buffer		buf;
...
/*
 *	sk_bt_relbuf() -- release a locked buffer.
 *
 * Lock and pin (refcount) are both dropped.
 */
void
sk_bt_relbuf(Relation rel, Buffer buf)
{
	UnlockReleaseBuffer(buf);
}

/*
 *	sk_bt_pageinit() -- Initialize a new page.
 *
 * On return, the page header is initialized; data space is empty;
 * special space is zeroed out.
 */
void
sk_bt_pageinit(Page page, Size size)
{
	PageInit(page, size, sizeof(SK_BTPageOpaqueData));
}

/*
 *	sk_bt_page_recyclable() -- Is an existing page recyclable?
 *
 * This exists to make sure sk_bt_getbuf and btvacuumscan have the same
 * policy about whether a page is safe to re-use.
 */
bool
sk_bt_page_recyclable(Page page)
{
 ...
 */
void
sk_bt_delitems_vacuum(Relation rel, Buffer buf,
 ...
 * Also, the given itemnos *must* appear in increasing order in the array.
 *
 * This is nearly the same as sk_bt_delitems_vacuum as far as what it does to
 * the page, but the WAL logging considerations are quite different.  See
 * comments for sk_bt_delitems_vacuum.
 */
void
sk_bt_delitems_delete(Relation rel, Buffer buf,
					OffsetNumber *itemnos, int nitems,
					Relation heapRel)
{
	Page		page = BufferGetPage(buf);
	...
	/*
	 * Unlike sk_bt_delitems_vacuum, we *must not* clear the vacuum cycle ID,
 ...
 */
static bool
sk_bt_is_page_halfdead(Relation rel, BlockNumber blk)
{
	Buffer		buf;
	Page		page;
	SK_BTPageOpaque opaque;
	...
	buf = sk_bt_getbuf(rel, blk, BT_READ);
	page = BufferGetPage(buf);
	opaque = (SK_BTPageOpaque) PageGetSpecialPointer(page);
 ...
 * indeed saves later search effort in sk_bt_pagedel.  The caller should
 ...
 */
static bool
sk_bt_lock_branch_parent(Relation rel, BlockNumber child, BTStack stack,
	...
	if (poffset >= maxoff)
	{
		...
		if (poffset == P_FIRSTDATAKEY(opaque))
		{
			...
			*rightsib = opaque->btpo_next;
			...
			/*
			 * Like in sk_bt_pagedel, check that the left sibling is not marked
			...
			if (leftsib != P_NONE)
			{
				...
				lbuf = sk_bt_getbuf(rel, leftsib, BT_READ);
			 ...
			 * sk_bt_mark_page_halfdead performed on the leaf level.
			 */
			if (sk_bt_is_page_halfdead(rel, *rightsib))
			{
			...
			return sk_bt_lock_branch_parent(rel, parent, stack->bts_parent,
...
/*
 * sk_bt_pagedel() -- Delete a page from the b-tree, if legal to do so.
 *
 ...
 */
int
sk_bt_pagedel(Relation rel, Buffer buf)
{
	#if 0
	int			ndeleted = 0;
	...
	for (;;)
	{
		...
		if (!P_ISHALFDEAD(opaque))
		{
			...
			if (!stack)
			{
				...
				itup_scankey = sk_bt_mkscankey(rel, targetkey);
				/* find the leftmost leaf page containing this key */
				stack = sk_bt_search(rel, rel->rd_rel->relnatts, itup_scankey,
			...
			if (!sk_bt_mark_page_halfdead(rel, buf, stack))
			{
				sk_bt_relbuf(rel, buf);
				return ndeleted;
			}
		 ...
		 * sk_bt_unlink_halfdead_page unlinks the topmost page from the branch,
		 * making it shallower.  Iterate until the leaf page is gone.
		 */
		rightsib_empty = false;
		while (P_ISHALFDEAD(opaque))
		{
			if (!sk_bt_unlink_halfdead_page(rel, buf, &rightsib_empty))
			{
				/* sk_bt_unlink_halfdead_page already released buffer */
				return ndeleted;
			}
			ndeleted++;
		}
		...
		buf = sk_bt_getbuf(rel, rightsib, BT_WRITE);
	}
 ...
 */
static bool
sk_bt_mark_page_halfdead(Relation rel, Buffer leafbuf, BTStack stack)
{
	 ...
	 */
	if (sk_bt_is_page_halfdead(rel, leafrightsib))
	{
		elog(DEBUG1, "could not delete page %u because its right sibling %u is half-dead",
	...
	target = leafblkno;
	if (!sk_bt_lock_branch_parent(rel, leafblkno, stack,
								&topparent, &topoff, &target, &rightsib))
		return false;
	 ...
	 * sk_bt_lock_branch_parent should have guaranteed it has the expected
 ...
 */
static bool
sk_bt_unlink_halfdead_page(Relation rel, Buffer leafbuf, bool *rightsib_empty)
{
	#if 0
	...
	if (ItemPointerIsValid(leafhikey))
	{
		...
		buf = sk_bt_getbuf(rel, target, BT_READ);
		page = BufferGetPage(buf);
		opaque = (SK_BTPageOpaque) PageGetSpecialPointer(page);
		leftsib = opaque->btpo_prev;
	...
	if (leftsib != P_NONE)
	{
		lbuf = sk_bt_getbuf(rel, leftsib, BT_WRITE);
	 ...
	 */
	rightsib = opaque->btpo_next;
	rbuf = sk_bt_getbuf(rel, rightsib, BT_WRITE);

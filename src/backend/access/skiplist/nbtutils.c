...
static Datum sk_bt_find_extreme_element(IndexScanDesc scan, ScanKey skey,
						 StrategyNumber strat,
						 Datum *elems, int nelems);
static int sk_bt_sort_array_elements(IndexScanDesc scan, ScanKey skey,
						bool reverse,
						Datum *elems, int nelems);
static int	sk_bt_compare_array_elements(const void *a, const void *b, void *arg);
static bool sk_bt_compare_scankey_args(IndexScanDesc scan, ScanKey op,
						 ScanKey leftarg, ScanKey rightarg,
						 bool *result);
static bool sk_bt_fix_scankey_strategy(ScanKey skey, int16 *indoption);
static void sk_bt_mark_scankey_required(ScanKey skey);
static bool sk_bt_check_rowcompare(ScanKey skey,
 ...
 *
 *		The result is intended for use with sk_bt_compare().
 */
ScanKey
sk_bt_mkscankey(Relation rel, IndexTuple itup)
{
	ScanKey		skey;
	TupleDesc	itupdesc;
	int			natts;
 ...
 *
 *		The result cannot be used with sk_bt_compare(), unless comparison
 *		data is first stored into the key entries.  Currently this
 *		routine is only called by nbtsort.c and tuplesort.c, which have
 *		their own comparison routines.
 */
ScanKey
sk_bt_mkscankey_nodata(Relation rel)
{
	ScanKey		skey;
	int			natts;
	int16	   *indoption;
...
/*
 * free a scan key made by either sk_bt_mkscankey or sk_bt_mkscankey_nodata.
 */
void
sk_bt_freeskey(ScanKey skey)
{
	pfree(skey);
}

/*
 * free a retracement stack made by sk_bt_search.
 */
void
sk_bt_freestack(SkiplistContext stack)
{
	pfree(stack);
}
...
/*
 *	sk_bt_preprocess_array_keys() -- Preprocess SK_SEARCHARRAY scan keys
 *
 * If there are any SK_SEARCHARRAY scan keys, deconstruct the array(s) and
 * set up SK_BTArrayKeyInfo info for each one that is an equality-type key.
 ...
 * on scan->keyData, is that callers are permitted to call sk_btrescan without
 * supplying a new set of scankey data.
 */
void
sk_bt_preprocess_array_keys(IndexScanDesc scan)
{
	...
	so->arrayKeys = (SK_BTArrayKeyInfo *) palloc0(numArrayKeys * sizeof(SK_BTArrayKeyInfo));
	...
	for (i = 0; i < numberOfKeys; i++)
	{
		...
		switch (cur->sk_strategy)
		{
			...
			case BTLessEqualStrategyNumber:
				cur->sk_argument =
					sk_bt_find_extreme_element(scan, cur,
		 ...
		 */
		num_elems = sk_bt_sort_array_elements(scan, cur,
											(indoption[cur->sk_attno - 1] & INDOPTION_DESC) != 0,
											elem_values, num_nonnulls);

		/*
		 * And set up the SK_BTArrayKeyInfo data.
		 */
...
/*
 * sk_bt_find_extreme_element() -- get least or greatest array element
 *
 ...
 */
static Datum
sk_bt_find_extreme_element(IndexScanDesc scan, ScanKey skey,
						 StrategyNumber strat,
						 Datum *elems, int nelems)
{
...
/*
 * sk_bt_sort_array_elements() -- sort and de-dup array elements
 *
 ...
 */
static int
sk_bt_sort_array_elements(IndexScanDesc scan, ScanKey skey,
						bool reverse,
						Datum *elems, int nelems)
{
	Relation	rel = scan->indexRelation;
	...
	qsort_arg((void *) elems, nelems, sizeof(Datum),
			  sk_bt_compare_array_elements, (void *) &cxt);
 ...
 */
static int
sk_bt_compare_array_elements(const void *a, const void *b, void *arg)
{
...
/*
 * sk_bt_start_array_keys() -- Initialize array keys at start of a scan
 *
 * Set up the cur_elem counters and fill in the first sk_argument value for
 * each array scankey.  We can't do this until we know the scan direction.
 */
void
sk_bt_start_array_keys(IndexScanDesc scan, ScanDirection dir)
{
	SK_BTScanOpaque so = (SK_BTScanOpaque) scan->opaque;
	int			i;
	...
	{
		SK_BTArrayKeyInfo *curArrayKey = &so->arrayKeys[i];
...
/*
 * sk_bt_advance_array_keys() -- Advance to next set of array elements
 *
 * Returns TRUE if there is another set of values to consider, FALSE if not.
 * On TRUE result, the scankeys are initialized with the next set of values.
 */
bool
sk_bt_advance_array_keys(IndexScanDesc scan, ScanDirection dir)
{
	SK_BTScanOpaque so = (SK_BTScanOpaque) scan->opaque;
	bool		found = false;
	int			i;
	...
	if (scan->parallel_scan != NULL)
		sk_bt_parallel_advance_array_keys(scan);

	return found;
}

/*
 * sk_bt_mark_array_keys() -- Handle array keys during sk_btmarkpos
 *
 * Save the current state of the array keys as the "mark" position.
 */
void
sk_bt_mark_array_keys(IndexScanDesc scan)
{
	SK_BTScanOpaque so = (SK_BTScanOpaque) scan->opaque;
	int			i;
...
/*
 * sk_bt_restore_array_keys() -- Handle array keys during sk_btrestrpos
 *
 * Restore the array keys to where they were when the mark was set.
 */
void
sk_bt_restore_array_keys(IndexScanDesc scan)
{
	SK_BTScanOpaque so = (SK_BTScanOpaque) scan->opaque;
	bool		changed = false;
	int			i;
	...
	/*
	 * If we changed any keys, we must redo sk_bt_preprocess_keys.  That might
...
/*
 *	sk_bt_preprocess_keys() -- Preprocess scan keys
 *
 ...
 * respectively.  sk_bt_checkkeys uses these flags.  For example, if the quals
 * are "x = 1 AND y < 4 AND z < 5", then sk_bt_checkkeys will reject a tuple
 ...
 * sk_bt_first will arbitrarily pick one of the keys to do the initial
 ...
 */
void
sk_bt_preprocess_keys(IndexScanDesc scan)
{
	SK_BTScanOpaque so = (SK_BTScanOpaque) scan->opaque;
	int			numberOfKeys = scan->numberOfKeys;
	int16	   *indoption = scan->indexRelation->rd_indoption;
	...
	if (numberOfKeys == 1)
	{
		...
		if (!sk_bt_fix_scankey_strategy(cur, indoption))
			so->qual_ok = false;
		memcpy(outkeys, cur, sizeof(ScanKeyData));
		so->numberOfKeys = 1;
		/* We can mark the qual as required if it's for first index col */
		if (cur->sk_attno == 1)
			sk_bt_mark_scankey_required(outkeys);
		return;
	}
	...
	for (i = 0;; cur++, i++)
	{
		...
		if (i == numberOfKeys || cur->sk_attno != attno)
		{
			...
			if (xform[BTEqualStrategyNumber - 1])
			{
				...
				for (j = BTMaxStrategyNumber; --j >= 0;)
				{
					...
					if (sk_bt_compare_scankey_args(scan, chk, eq, chk,
												 &test_result))
					{
						if (!test_result)
						{
				...
				&& xform[BTLessEqualStrategyNumber - 1])
			{
				...
				if (sk_bt_compare_scankey_args(scan, le, lt, le,
											 &test_result))
				{
					if (test_result)
				...
				&& xform[BTGreaterEqualStrategyNumber - 1])
			{
				...
				if (sk_bt_compare_scankey_args(scan, ge, gt, ge,
											 &test_result))
				{
					if (test_result)
			...
			for (j = BTMaxStrategyNumber; --j >= 0;)
			{
				if (xform[j])
				{
					...
					if (priorNumberOfEqualCols == attno - 1)
						sk_bt_mark_scankey_required(outkey);
				}
			}
		...
		else
		{
			...
			if (sk_bt_compare_scankey_args(scan, cur, cur, xform[j],
										 &test_result))
			{
				if (test_result)
					xform[j] = cur;
 ...
 */
static bool
sk_bt_compare_scankey_args(IndexScanDesc scan, ScanKey op,
						 ScanKey leftarg, ScanKey rightarg,
						 bool *result)
{
	Relation	rel = scan->indexRelation;
	 ...
	 * operators.)
	 *
	 * If the sk_strategy was flipped by sk_bt_fix_scankey_strategy, we have to
 ...
 */
static bool
sk_bt_fix_scankey_strategy(ScanKey skey, int16 *indoption)
{
	int			addflags;
 ...
 */
static void
sk_bt_mark_scankey_required(ScanKey skey)
{
	int			addflags;
 ...
 * accordingly.  See comments for sk_bt_preprocess_keys(), above, about how
 * this is done.
 *
 ...
 */
IndexTuple
sk_bt_checkkeys(IndexScanDesc scan,
			  ...
			  ScanDirection dir, bool *continuescan)
{
	...
	*continuescan = true;		/* default assumption */
	...
	so = (SK_BTScanOpaque) scan->opaque;
	...
	for (key = so->keyData, ikey = 0; ikey < keysz; key++, ikey++)
	{
		...
		{
			if (sk_bt_check_rowcompare(key, tuple, tupdesc, dir, continuescan))
				continue;
			return NULL;
		}
		...
		if (!DatumGetBool(test))
		{
			 ...
			 * initial positioning in sk_bt_first() when they are available. See
			 * comments in sk_bt_first().
			 */
			if ((key->sk_flags & SK_BT_REQFWD) &&
				ScanDirectionIsForward(dir))
				*continuescan = false;
 ...
 * to pass the qual.
 *
 * This is a subroutine for sk_bt_checkkeys, which see for more info.
 */
static bool
sk_bt_check_rowcompare(ScanKey skey, IndexTuple tuple, TupleDesc tupdesc,
					 ScanDirection dir, bool *continuescan)
{
	ScanKey		subkey = (ScanKey) DatumGetPointer(skey->sk_argument);
	int32		cmpresult = 0;
...
/*
 * sk_bt_killitems - set LP_DEAD state for items an indexscan caller has
 * told us were killed
 *
 ...
 */
void
sk_bt_killitems(IndexScanDesc scan)
{
	SK_BTScanOpaque so = (SK_BTScanOpaque) scan->opaque;
	Page		page;
	...
	else
	{
		...
		buf = sk_bt_getbuf(scan->indexRelation, so->currPos.currPage, BT_READ);
		...
		else
		{
			...
			sk_bt_relbuf(scan->indexRelation, buf);
			return;
		}
	}
	...
	for (i = 0; i < numKilled; i++)
	{
		...
		SK_BTScanPosItem *kitem = &so->currPos.items[itemIndex];
...
typedef struct BTOneVacInfo
{
	...
	SK_BTCycleId	cycleid;		/* cycle ID for its active VACUUM */
} BTOneVacInfo;

typedef struct BTVacInfo
{
	SK_BTCycleId	cycle_ctr;		/* cycle ID most recently assigned */
...
/*
 * sk_bt_vacuum_cycleid --- get the active vacuum cycle ID for an index,
 ...
 */
SK_BTCycleId
sk_bt_vacuum_cycleid(Relation rel)
{
	SK_BTCycleId	result = 0;
	int			i;
...
/*
 * sk_bt_start_vacuum --- assign a cycle ID to a just-starting VACUUM operation
 *
 * Note: the caller must guarantee that it will eventually call
 * sk_bt_end_vacuum, else we'll permanently leak an array slot.  To ensure
 ...
 */
SK_BTCycleId
sk_bt_start_vacuum(Relation rel)
{
	SK_BTCycleId	result;
	int			i;
	BTOneVacInfo *vac;
	...
	for (i = 0; i < btvacinfo->num_vacuums; i++)
	{
			...
			vac->relid.dbId == rel->rd_lockInfo.lockRelId.dbId)
		{
			 ...
			 * we expect sk_bt_end_vacuum() to be called before transaction
...
/*
 * sk_bt_end_vacuum --- mark a btree VACUUM operation as done
 *
...
/*
 * sk_bt_end_vacuum wrapped as an on_shmem_exit callback function
 */
void
sk_bt_end_vacuum_callback(int code, Datum arg)
{
	sk_bt_end_vacuum((Relation) DatumGetPointer(arg));
}

/*
 * sk_BTreeShmemSize --- report amount of shared memory space needed
 */
Size
sk_BTreeShmemSize(void)
{
	Size		size;
...
/*
 * sk_BTreeShmemInit --- initialize this module's shared memory
 */
void
sk_BTreeShmemInit(void)
{
	bool		found;
	...
	if (!IsUnderPostmaster)
	{
		 ...
		 */
		btvacinfo->cycle_ctr = (SK_BTCycleId) time(NULL);
...
bytea *
sk_btoptions(Datum reloptions, bool validate)
{
	return default_reloptions(reloptions, validate, RELOPT_KIND_BTREE);
}

/*
 *	sk_btproperty() -- Check boolean properties of indexes.
 *
 * This is optional, but handling AMPROP_RETURNABLE here saves opening the rel
 * to call sk_btcanreturn.
 */
bool
sk_btproperty(Oid index_oid, int attno,
